...
# By-blocks execution
n_samples=len(os.listdir(os.path.join(os.getcwd(), 'simulations_sphere/' 'data_unif')))
...
def task(file) -> None:
    ...
    results_filename = os.path.join(os.getcwd(), 'simulations_sphere', 'results_unif', f'{file[:-4]}' + '_results.npy')
    if os.path.exists(results_filename):
        print(f"Results file already exists for {file}, skipping...")
        return

    with open(os.path.join(os.getcwd(), 'simulations_sphere', 'data_unif', file), 'rb') as f:
    ...
    with open(os.path.join(os.getcwd(), 'simulations_sphere', 'type_i_data_unif', type_i_filename), 'rb') as f:
...
############################################################################################################            
    ...
    theta_samples = np.array([np.random.uniform(0, 1, MC)]).reshape(-1, 1)
...
############################################################################################################
    ...
    with open(os.path.join(os.getcwd(), 'simulations_sphere', 'type_iii_data_unif', type_iii_filename), 'rb') as f:
...
###########################################################################################################
    ...
    theta = np.repeat(0.25, MC)  # Using uniform equivalent of 25th percentile
...
file_list = list(filter(
                ...
                filter(lambda file: file.endswith('.pkl'), os.listdir(os.path.join(os.getcwd(), 'simulations_sphere', 'data_unif/')))
                    )
                )  
