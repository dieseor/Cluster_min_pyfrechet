...
save_folder = os.path.join(os.getcwd(), 'simulations_sphere', 'type_iii_data_unif')
os.makedirs(save_folder, exist_ok=True)

for sample_size in sample_sizes:
    for kappa in kappa_values:
        theta = np.array([0.25])
        theta_samples = np.repeat(theta, M, axis=0)  # Repeat theta for sample size
        theta_samples, new_ys = simulate_data(kappa = kappa, mu = mu, theta_samples = theta_samples)

        filename = os.path.join(save_folder, f'sphere_type_iii_N{sample_size}_kappa{kappa}.pkl')
        with open(filename, 'wb') as f:
            pickle.dump({'theta': theta_samples, 'Y': new_ys}, f)