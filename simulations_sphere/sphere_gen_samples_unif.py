...
save_folder = os.path.join(os.getcwd(), 'simulations_sphere', 'data_unif')
os.makedirs(save_folder, exist_ok=True)

for sample_size in sample_sizes:
    for kappa in kappa_values:
        for k in range(1, n_samples + 1):
            theta_samples = np.random.uniform(0, 1, sample_size) # Random angles from uniform(0,1)
            theta, YY = simulate_data(kappa, mu, theta_samples)
            Y = YY  # Use all samples, not just the first one
