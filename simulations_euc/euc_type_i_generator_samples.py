...
def save_simulated_samples(n_samples, sample_sizes, n_predictors, sigma_values, betas):
    ...
    save_folder = os.path.join(os.getcwd(), 'simulations_euc', 'type_i_data')
    ...
    for sample_size in sample_sizes:
        for sigma in sigma_values:
                ...
                X_design = 2*np.sqrt(5)*(np.random.beta(2, 2, (M, n_predictors)) - 1/2)
                X, Y = simulate_data(sigma=sigma, X_design=X_design, betas=betas)

                # Define the filename for saving
                filename = os.path.join(save_folder, f'euc_type_i_N{sample_size}_sigma{np.round(sigma, 1)}.pkl')
