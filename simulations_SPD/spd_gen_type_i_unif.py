...
save_folder = os.path.join(os.getcwd(), 'simulations_SPD', 'type_i_data_unif')
...
for sample_size in sample_sizes:
    for df in dfs:
        # generate sample_t from a uniform distribution
        sample_t = np.random.uniform(0, 1, M)
        sample = sim_regression_matrices(Sigmas = (Sigma_1, Sigma_2, Sigma_3), 
                                        t = sample_t,  
                                        df = df)
        
        filename = os.path.join(save_folder, f'SPD_type_i_N{sample_size}_df{df}.pkl')
        with open(filename, 'wb') as f:
            pickle.dump(sample, f)
        print(f"Saved sample for N={sample_size}, df={df} to {filename}")