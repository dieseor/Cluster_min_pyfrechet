...
save_folder = os.path.join(os.getcwd(), 'simulations_SPD', 'data_unif')
...
for sample_size in sample_sizes:
    for df in dfs:
        for k in range(1, n_samples+1):
            sample_t = np.random.uniform(0, 1, sample_size)
            sample = sim_regression_matrices(Sigmas = (Sigma_1, Sigma_2, Sigma_3), 
                                           t = sample_t,  
                                           df = df)
            
            filename = os.path.join(save_folder, f'SPD_samp{k}_N{sample_size}_df{df}_block_{(k-1) // 10 + 1}.pkl')
